...
describe('seedTestimonialFromProfilePhotos', () => {
  it('does not invent a testimonial from profile left', () => {
    assert.equal(
      seedTestimonialFromProfilePhotos(null, { left: 'https://cdn.example/left.jpg' }),
      null,
    );
  });

  it('returns the existing testimonial unchanged', () => {
    const row = {
      before_image_path: '99/real.jpg',
      after_image_path: '99/after.jpg',
      status: 'pending',
    };
    const seeded = seedTestimonialFromProfilePhotos(row, {
      left: 'https://cdn.example/left.jpg',
    });
    assert.equal(seeded, row);
    assert.equal(seeded.before_image_path, '99/real.jpg');
  });
});

describe('memberHasVisibleTransformationPhoto', () => {
  it('does not count profile left alone', () => {
    assert.equal(
