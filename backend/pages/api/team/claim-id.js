...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Team ID is now OPTIONAL - if not provided, just return success
    if (!teamId || teamId.trim() === "") {
      res.status(200).json({
        success: true,
        message: "Skipped Team ID setup",
        skipped: true,
    ...
    const activeCoachTeams = allCoachTeams
      ? allCoachTeams.filter((team) => team.Status === "active")
      : [];
    ...
    if (existingTeamId && existingTeamId.length > 0) {
      ...
      if (activeCoachTeams.length > 0) {
        ...
        allCoachTeams &&
        allCoachTeams.length > 0 &&
        allCoachTeams[0].Status === "inactive"
      ) {
    ...
    res.status(200).json({
      success: true,
      message: isReactivatingTeam
        ? "Team ID claimed (was inactive)"
        : isJoiningExistingTeam
          ? "Team ID claimed (will join as co-coach)"
