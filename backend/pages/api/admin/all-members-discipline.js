  ...
  try {
    ...
    memberIds.forEach(userId => {
      disciplineData[userId] = {
        userId,
        weight: { dates: new Set(), onTimeDates: new Set(), totalPosts: 0, onTimePosts: 0 },
        education: { dates: new Set(), onTimeDates: new Set(), totalPosts: 0, onTimePosts: 0 },
        breakfast: { dates: new Set(), onTimeDates: new Set(), totalPosts: 0, onTimePosts: 0 },
        lunch: { dates: new Set(), onTimeDates: new Set(), totalPosts: 0, onTimePosts: 0 },
        dinner: { dates: new Set(), onTimeDates: new Set(), totalPosts: 0, onTimePosts: 0 }
      };
      
      // Weight
      const weightWindow = timeWindowMap.weight || { start: '05:00:00', end: '09:00:00' };
      disciplineData[userId].weight.dates = getUniqueDates(weightData.data || [], userId);
      disciplineData[userId].weight.onTimeDates = getUniqueOnTimeDates(
        weightData.data || [], userId, weightWindow.start, weightWindow.end
      );
      disciplineData[userId].weight.totalPosts = disciplineData[userId].weight.dates.size;
      disciplineData[userId].weight.onTimePosts = disciplineData[userId].weight.onTimeDates.size;
      
      // Education
      const educationWindow = timeWindowMap.education || { start: '05:00:00', end: '23:00:00' };
      disciplineData[userId].education.dates = getUniqueDates(educationData.data || [], userId);
      ...
      disciplineData[userId].education.totalPosts = disciplineData[userId].education.dates.size;
      disciplineData[userId].education.onTimePosts = disciplineData[userId].education.onTimeDates.size;
          ...
          foodData.data.forEach(r => {
            if (r.UserID == userId) {
              ...
              if (time >= mealWindow.start && time <= mealWindow.end) {
                ...
                onTimeDates.add(date.toISOString().split('T')[0]);
              }
            }
          });
        }
      ...
      disciplineData[userId].breakfast.onTimePosts = breakfastData.onTimeDates.size;
    ...
    const avgDiscipline = formattedMembers.length > 0
      ? formattedMembers.reduce((sum, m) => sum + m.periodDiscipline.percentage, 0) / formattedMembers.length
      : 0;
    
    const totalCoaches = formattedMembers.filter(m => m.role === 'coach').length;
    
    const topPerformer = formattedMembers.length > 0
      ? formattedMembers.reduce((top, m) => 
          m.periodDiscipline.percentage > top.periodDiscipline.percentage ? m : top
        )
      : null;
    
    const needsAttention = formattedMembers
      .filter(m => m.periodDiscipline.percentage < 60)
      .sort((a, b) => a.periodDiscipline.percentage - b.periodDiscipline.percentage)
      .slice(0, 5);
