...
export default async function handler(req, res) {
  ...
  try {
    ...
    // BMR is now stored in team_table — not carried forward in weight records
    // Only use BMR if explicitly provided in the request (e.g. from weight scale reading)
    let bmrValue = bmr && !isNaN(parseFloat(bmr)) ? parseFloat(bmr) : null;
    if (bmrValue) {
      console.log("✏️ [BMR] Explicitly provided in weight entry:", bmrValue);

      // ── Sync BMR to team_table so profile page + activity report see the latest value ──
      try {
