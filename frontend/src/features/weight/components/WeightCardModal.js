...
const WeightCardModal = ({ data, onClose, onDelete, onUpdate, previousWeight = null, apiBaseUrl, userId = null }) => {
  ...
  // Γ£à Lazy-fetched scale image (list endpoint omits base64 for speed)
  ...
  // Γ£à Fetch full-size image on demand when modal opens (if not already provided)
  useEffect(() => {
    if (!data?.ID) return;
    ...
    (async () => {
      try {
        const res = await fetch(
          `${apiBaseUrl}/api/weight/image?userId=${encodeURIComponent(userId)}&id=${encodeURIComponent(data.ID)}`
        );
        if (!res.ok) return;
