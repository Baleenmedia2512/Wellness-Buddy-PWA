// OnboardingIdentityPage — Welcome: display name only.
// Email verification + Community ID live on Home → Profile (KYC-style).
...
import { looksLikeEmail } from '../domain/onboardingEmail.js';
 ...
 * @param {{
 *   user: object,
 *   onComplete: (saved: { userName: string, email?: string }) => void | Promise<void>,
 * }} props
 */
const OnboardingIdentityPage = ({ user, onComplete }) => {
  ...
  const handleContinue = useCallback(async () => {
    ...
    try {
      ...
      await onComplete?.({ userName: trimmedName });
    } catch (e) {
      setError(e?.message || 'Could not save. Please try again.');
    } finally {
      setSaving(false);
    }
  }, [name, nameValid, emailLocked, loginEmail, uid, onComplete]);
  ...
  return (
    <div className="fixed inset-0 z-[80] bg-gray-50 flex flex-col">
      <div className="bg-gradient-to-r from-green-600 to-green-700 px-4 pt-10 pb-6 text-white">
        ...
        <p className="text-sm text-green-100 mt-1">What should we call you?</p>
      </div>
