...
// Blocking modal — forces upload of a profile picture (any photo accepted).
...
const MandatoryProfilePictureModal = ({ user, onComplete }) => {
  ...
  const cropper = useImageCropper({
    ...
    onCropped: (cropped) => {
<<<<<<< HEAD
      setProfileImage(cropped); setPreviewUrl(cropped); face.reset(); face.run(cropped, user?.id ?? null, 'PROFILE_IMAGE_SET');
=======
      setProfileImage(cropped); setPreviewUrl(cropped);
    ...
    <div className="fixed inset-0 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" style={{ zIndex: 350 }}>
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full">
        ...
        <div className="p-6 space-y-5">
          ...
          <PicturePreview previewUrl={previewUrl} faceStatus="idle"
