...
const FaceStatusBadge = ({ status }) => {
  ...
  const cfg = {
    ...
    detection_error:  { cls: 'bg-yellow-50 border-yellow-200 text-yellow-700', icon: <AlertCircle className="w-4 h-4 flex-shrink-0" />,      msg: 'Could not verify photo. You may still try saving.' },
  }[status];
  if (!cfg) return null;
  return (
