...
const CameraGalleryButtons = ({ onCameraSelect, onGallerySelect, disabled, layout = 'wide' }) => {
  ...
  const baseBtn = layout === 'compact'
    ...
    : inSlot
      ? 'flex flex-col items-center justify-center p-3 border-2 rounded-xl transition-colors disabled:opacity-50 disabled:cursor-not-allowed bg-white'
  ...
  return (
    <div className="flex justify-center w-full">
      ...
      <button type="button" disabled={disabled} onClick={() => galleryRef.current?.click()}
        ...
        <Upload className={layout === 'compact' ? 'w-6 h-6' : inSlot ? 'w-7 h-7 mb-1.5' : 'w-8 h-8 mb-2'} />
        <span className={layout === 'compact' ? '' : `text-sm font-medium text-green-700 ${inSlot ? 'text-xs' : ''}`}>{layout === 'compact' ? 'From Gallery' : 'Upload Photo'}</span>
      </button>
    </div>
    </div>
  );
