...
const extractFoodNames = (analysisResult) => {
  ...
  try {
    ...
    if (
      analysisResult?.detailedItems &&
      Array.isArray(analysisResult.detailedItems)
    ) {
...
const parseAnalysisData = (analysisData) => {
  try {
    ...
    if (
      Array.isArray(parsed.detailedItems) &&
      parsed.detailedItems.length > 0
    ) {
...
export async function checkForDuplicateFood({ userId, analysisResult }) {
  try {
    ...
    if (!response.ok) {
      console.error(
        "Failed to fetch nutrition stats, status:",
        response.status,
      );
      return { isDuplicate: false };
    }
    ...
    // Collect ALL duplicate foods instead of just the first one
    ...
    for (const meal of mealsInCurrentSlot) {
      ...
      for (const newFoodName of newFoodNames) {
        ...
        if (
          duplicateFoods.some(
            (d) => d.toLowerCase() === newFoodName.toLowerCase(),
          )
        )
          continue;
        ...
        for (const existingFoodName of existingFoodNames) {
          ...
          if (!existingFoodName || typeof existingFoodName !== "string")
            continue;

          // Simple exact match (case-insensitive)
          if (newFoodName === existingFoodName) {
            duplicateFoods.push(newFoodName);
            break; // Found duplicate, move to next new food
          }
        }
      }
    }

    // If duplicates found, return all of them
    if (duplicateFoods.length > 0) {
      // Create display name based on number of duplicates
      let displayName;
      if (duplicateFoods.length === 1) {
        displayName = duplicateFoods[0];
      } else if (duplicateFoods.length === 2) {
        displayName = duplicateFoods.join(" and ");
      } else {
        displayName =
          duplicateFoods.slice(0, -1).join(", ") +
          " and " +
          duplicateFoods[duplicateFoods.length - 1];
      }

      return {
        isDuplicate: true,
        duplicateFoodName: displayName,
        duplicateFoodNames: duplicateFoods, // Array of all duplicates
        duplicateCount: duplicateFoods.length,
        mealType: mealCategoryName,
        allFoodNames: newFoodNames, // All foods in the new upload
      };
    }
...
export async function checkForDuplicateWeight({
  ...
  try {
    ...
    if (!userId || (typeof userId !== "string" && typeof userId !== "number")) {
      console.warn(
        "Invalid userId provided to weight duplicate check:",
        userId,
      );
      return { isDuplicate: false };
    }
    ...
    if (!weightValue || isNaN(parseFloat(weightValue))) {
      console.warn(
        "Invalid weight value provided to duplicate check:",
        weightValue,
      );
      return { isDuplicate: false };
    }
    ...
    if (!response.ok) {
      console.error(
        "Failed to fetch weight history for duplicate check, status:",
        response.status,
      );
      return { isDuplicate: false };
    }
    ...
    for (const entry of todayEntries) {
      ...
      if (weightDiff <= tolerance) {
        ...
        try {
          ...
          let timeDifference;
          if (timeDiffHours > 0) {
            timeDifference = `${timeDiffHours} hour${
              timeDiffHours > 1 ? "s" : ""
            } ago`;
          } else if (timeDiffMinutes > 0) {
            timeDifference = `${timeDiffMinutes} minute${
              timeDiffMinutes > 1 ? "s" : ""
            } ago`;
          } else {
