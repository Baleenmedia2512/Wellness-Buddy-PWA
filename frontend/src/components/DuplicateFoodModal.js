...
const DuplicateFoodModal = ({ foodName, mealType, onConfirm, onCancel, isWeight, weightValue, unit, timeDifference }) => {
  ...
  if (isWeight) {
    if (weightValue == null || !unit || !timeDifference) {
      console.error('DuplicateFoodModal (Weight): Missing required props', { weightValue, unit, timeDifference });
      return null;
    }
  } else {
    if (!foodName || !mealType) {
      console.error('DuplicateFoodModal (Food): Missing required props', { foodName, mealType });
      return null;
    }
  }
  
  // Edge case: Validate callback functions
  const safeOnConfirm = typeof onConfirm === 'function' ? onConfirm : () => console.error('onConfirm not provided');
  const safeOnCancel = typeof onCancel === 'function' ? onCancel : () => console.error('onCancel not provided');
  ...
  return (
    ...
    >
      ...
      >
        <div className={`${isWeight ? 'bg-gradient-to-br from-blue-500 to-indigo-600' : 'bg-gradient-to-br from-amber-500 to-orange-600'} p-6 text-white`}>
          ...
          <h2 id="duplicate-modal-title" className="text-2xl font-bold text-center">
            {isWeight ? 'Weight Detected' : 'Food Detected'}
          </h2>
        </div>

        <div className="p-6 space-y-4">
          <div className={`${isWeight ? 'bg-blue-50 border border-blue-200' : 'bg-amber-50 border border-amber-200'} rounded-xl p-4`}>
            {isWeight ? (
          ...
          <div className="flex gap-3 pt-2">
            <button
              ...
              aria-label={`Cancel and don't add duplicate ${isWeight ? 'weight' : 'food'}`}
            ...
            <button
              ...
              disabled={isProcessing}
              aria-label={`Confirm and add duplicate ${isWeight ? 'weight' : 'food'}`}
