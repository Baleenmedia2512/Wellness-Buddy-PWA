...
const AttendanceReport = ({ user, onBack }) => {
  ...
  const renderStatus = (node, showDetails) => {
    ...
    return (
      <div className="flex items-center gap-1 px-2 py-1 rounded-full bg-green-50 border border-green-300 text-green-700">
        ...
        <span className="text-[11px] font-semibold">Attended</span>
      </div>
    );
  };
    ...
    return (
      <>
        ...
        <div className="flex-1 flex flex-col items-center px-2">
          <span className="text-[9px] font-bold uppercase tracking-wide text-gray-400 mb-0.5">Direct</span>
          <div className="flex items-baseline gap-0.5">
            <span className="text-base font-bold text-gray-900">
              {totalDirect > 0 ? Math.round((attendedDirect / totalDirect) * 100) : 0}%
            </span>
          </div>
        ...
        <div className="flex-1 flex flex-col items-center pl-2">
          <span className="text-[9px] font-bold uppercase tracking-wide text-gray-400 mb-0.5">Full Team</span>
          <div className="flex items-baseline gap-0.5">
            <span className="text-base font-bold text-gray-900">
    ...
    ? {
        ...
        percentage:
          hierarchyData.fullTeamCount?.total > 0
            ? Math.round(
                (hierarchyData.fullTeamCount.qualified / hierarchyData.fullTeamCount.total) * 100
              )
            : 0,
      }
    : null;
  ...
  return (
    <HierarchicalReportLayout
      ...
      subtitle={`${teamCounts.coaches + teamCounts.members} Members • Last updated ${new Date().toLocaleString('en-US', {
        month: 'short',
        day: 'numeric',
        hour: 'numeric',
        minute: '2-digit',
