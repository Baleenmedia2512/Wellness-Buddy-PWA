...
const WellnessScoreReport = lazy(() =>
  import("./features/reports").then((m) => ({ default: m.WellnessScoreReport })),
);
...
function WellnessValleyApp() {
  ...
  const [showWellnessScoreReport, setShowWellnessScoreReport] = useState(false);
  ...
  useEffect(() => {
    const handlePopState = (event) => {
      ...
      if (page === 'main') {
        ...
        setShowWellnessScoreReport(false);
        ...
        Session.setCurrentPage('main');
      } else if (page === 'wellness-score-report') {
        bumpTabVisitKeyRef.current('wellness-score-report');
        setShowReports(false);
        startTransition(() => setShowWellnessScoreReport(true));
        Session.setCurrentPage('main');
  ...
  useEffect(() => {
    ...
    initializeBackButton(
      goBack,
      showToast,
      !showDashboard && !showWellnessCounselling && !showUniversityEnrollment && !showNutritionCentersMap && !showActivityReport && !showActivityTimeReport && !showTestimonials && !showReports && !showWellnessScoreReport && !showWellnessScoreSetup && !showAiCreditsSetup && !showManualEntry && !showWellnessScore && !showProfilePage,
    );
  ...
  const navigateTo = useCallback((targetPage) => {
    ...
    switch (targetPage) {
      ...
      case 'wellness-score-report':
        setShowWellnessScoreReport(true);
        break;
      case 'profile':
        setShowProfilePage(true);
        break;
  ...
  const buildAnalysisFromManualFood = (m) => buildManualFoodAnalysis(m);
  ...
  const handleUnknownShareRetry = async () => {
    ...
    try {
      if (creditsEnabled) {
        const reserved = await reserveAiCredit({ userId: user.id, apiBaseUrl });
  ...
  /**
   * Background AI after user taps Analyze on the capture classify sheet.
   * Closes the sheet first; results land in Diary (analyzing badge → typed row).
   * Credits: confirm only on successful food recognition; otherwise release.
   */
  const startBackgroundCaptureAi = useCallback(
    ({ captureId, imageBase64, userId: uid, reservationId = null }) => {
      ...
      triggerNutritionRefresh({ immediate: true, source: 'capture-ai-started' });

      void (async () => {
        const pendingSharePromise = Promise.resolve({ id: captureId });
        let file;
        try {
        ...
        try {
          ...
          updatePendingCaptureType(pendingSharePromise, 'unknown');
          clearCaptureAnalyzing(captureId);
          triggerNutritionRefresh({ immediate: true, source: 'capture-analysis-failed' });
          return;
        }
        ...
        try {
          ...
          if (detectedType?.type === 'education') {
          ...
          if (detectedType?.type === 'smartwatch') {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- App helpers / setters are stable for this capture lifecycle
    [
      user,
      apiBaseUrl,
      markCaptureAnalyzing,
    ...
    homeOverlay = (
      <div className="ios-full-page bg-gray-50">
        <Header
          ...
          onShowWellnessScoreReport={() => navigateTo('wellness-score-report')}
        />
  ...
  } else if (showManualEntry && manualEntryPayload) {
    homeOverlay = (
    ...
    homeOverlay = (
      <div className="ios-full-page bg-gray-50">
        <Header
          ...
          user={user}
          userRole={userRole}
          activePage="wellness-score-report"
          onShowHome={() => navigateTo('home')}
          onShowBackgroundHistory={() => navigateTo('dashboard')}
          onShowWellnessEnrollment={() => navigateTo('enrollment')}
          onShowWellnessCounselling={() => navigateTo('counselling')}
        ...
        />
        <div className="ios-scroll-body">
          <Suspense fallback={<LoadingSpinner message="Loading Wellness Score Report..." />}>
            <WellnessScoreReport
              user={user}
              tabVisitKey={tabVisitKeys['wellness-score-report'] ?? 0}
              onBack={() => {
                setShowWellnessScoreReport(false);
                const currentWvPage = window.history.state?.wvPage;
                if (currentWvPage && currentWvPage !== 'main') window.history.back();
      ...
      >
        ...
        <Header
          ...
          activePage={
            ...
            showReports ? 'reports' :
            showWellnessScoreReport ? 'wellness-score-report' :
