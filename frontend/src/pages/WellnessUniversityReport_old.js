...
import WellnessUniversityEnrollment from "./WellnessUniversityEnrollment";
...
const WellnessUniversityReport = ({ onClose, user, userRole }) => {
  ...
  const [showEnrollmentModal, setShowEnrollmentModal] = useState(false); // Edit enrollment modal
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="bg-gradient-to-r from-green-50 to-teal-50 rounded-xl p-4 border border-green-200">
          <h3 className="text-lg font-bold text-gray-800 mb-1">📋 Your Enrollment</h3>
          {enrollmentDate && (
            <p className="text-sm text-gray-600">Enrolled on: {enrollmentDate}</p>
          )}
        </div>
        ...
        {enrolledPrograms.length > 0 ? (
          <div className="space-y-2">
            {enrolledPrograms.map((programName) => {
              ...
              return (
                <div
                  key={programName}
                  className="flex items-center gap-3 bg-white p-3 rounded-lg border border-green-200 shadow-sm"
                >
                  ...
                  <div className="flex-1">
                    <p className="font-semibold text-gray-800 text-sm">{programName}</p>
                  </div>
          ...
          <div className="bg-gray-50 rounded-xl p-8 text-center">
            ...
            <p className="text-gray-600 font-medium">No programs enrolled yet</p>
            <p className="text-gray-500 text-sm mt-1">Click "Edit Programs" to get started!</p>
          </div>
        )}
  ...
  // Render member enrollment view (when searching)
  const renderMemberEnrollment = () => {
    if (!selectedMember) return null;

    const enrolledPrograms = memberEnrollment ? (() => {
      try {
        const parsed = JSON.parse(memberEnrollment.EnrolledPrograms || "[]");
        // Handle both array format and object format (with timestamps)
    ...
    const getProgramTimestamp = (programName) => {
      if (!memberEnrollment) return null;
      try {
        const parsed = JSON.parse(memberEnrollment.EnrolledPrograms || "[]");
        if (typeof parsed === 'object' && !Array.isArray(parsed)) {
          return parsed[programName];
        }
      } catch {}
      return null;
    };
      ...
      <div className="space-y-4">
        ...
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-4 border border-blue-200">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <h3 className="text-lg font-bold text-gray-800">👤 {selectedMember.name}'s Enrollment</h3>
              <p className="text-sm text-gray-600">{selectedMember.email}</p>
            </div>
          </div>
        </div>

        {/* Programs List - Show ALL programs (enrolled + not enrolled) */}
        <div className="space-y-2">
          {PROGRAMS.map((program) => {
            const isEnrolled = enrolledPrograms.includes(program.name);
            const timestamp = getProgramTimestamp(program.name);
            ...
            return (
              ...
              >
                <div className={`w-10 h-10 flex-shrink-0 border-2 rounded-xl flex items-center justify-center text-2xl ${
                  isEnrolled
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 text-center">
          <p className="text-yellow-800 text-sm font-medium">👁️ View Only - You cannot edit this enrollment</p>
        </div>
      </div>
    );
  };
