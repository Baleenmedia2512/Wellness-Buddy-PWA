...
const SetupWizard = ({
  ...
  useEffect(() => {
    ...
    console.log("🚀 SetupWizard mounted");
    console.log("🔍 userEmail from PROP:", propUserEmail);
    console.log("🔍 userEmail in localStorage:", localEmail);
    console.log("🔍 Final userEmail to use:", propUserEmail || localEmail);
  ...
  const checkTeamIdAvailability = async () => {
    // Skip check if Team ID is empty (user wants to skip)
    ...
    try {
      const userEmail = propUserEmail || localStorage.getItem("userEmail");
      console.log(
        "🔍 checkTeamIdAvailability - userEmail from prop:",
        propUserEmail,
      );
      console.log(
        "🔍 checkTeamIdAvailability - userEmail from localStorage:",
        localStorage.getItem("userEmail"),
      );
      console.log("🔍 checkTeamIdAvailability - final userEmail:", userEmail);
      console.log("🔍 checkTeamIdAvailability - checking teamId:", teamId);

      if (!userEmail) {
        console.error("❌ No userEmail available!");
        setError("Session expired. Please login again.");
        setCheckingTeamId(false);
        return;
      }

      console.log("📡 Calling check-availability API...");
      const response = await axios.get(
  ...
  const claimTeamIdAndSendRequest = async () => {
    // If no Team ID entered and no coach selected, just skip everything
    if (!teamId && !selectedCoach) {
      console.log("⏭️ No Team ID and no coach - skipping setup completely");
    ...
    try {
      const userEmail = propUserEmail || localStorage.getItem("userEmail");
      console.log(
        "🔍 claimTeamIdAndSendRequest - userEmail from prop:",
        propUserEmail,
      );
      console.log(
        "🔍 claimTeamIdAndSendRequest - userEmail from localStorage:",
        localStorage.getItem("userEmail"),
      );
      console.log("🔍 claimTeamIdAndSendRequest - final userEmail:", userEmail);
      console.log(
        "🔍 claimTeamIdAndSendRequest - teamId:",
        teamId || "(empty - will skip)",
      );
      console.log(
        "🔍 claimTeamIdAndSendRequest - selectedCoach:",
        selectedCoach,
      );
      ...
      console.log("📡 Claiming Team ID:", {
        teamId: teamId || "(skipped)",
        email: userEmail,
      });

      // Step 1: Claim Team ID (optional - can be empty)
      const claimResponse = await axios.post(`${API_BASE}/api/team/claim-id`, {
        teamId: teamId || "",
        email: userEmail,
      });

      console.log("Team ID claim response:", claimResponse.data);
        ...
        <div className="shrink-0">
          ...
          <div className="px-8 text-center mb-8">
            ...
            <p className="text-gray-500 text-sm leading-relaxed">
              Complete these 2 simple steps to join your team and activate your
              account.
            </p>
          </div>
              ...
              >
                <div className="mb-6">
                  ...
                  <p className="text-gray-500 text-sm mb-4">
                    Search for the person who invited you to Wellness Valley.
                    They will be your mentor.
                  </p>
            ...
            {step === 2 && (
              ...
              >
                ...
                <div className="mb-6">
                  ...
                  <p className="text-gray-500 text-sm mb-4">
                    Enter your unique 10-character team identifier, or skip to
                        ...
                        teamIdInfo?.existingCoach && (
                          <div className="bg-green-50 border border-green-100 rounded-xl p-3 text-left">
                            ...
                            <div className="flex items-center gap-2 bg-white px-3 py-1.5 rounded-lg border border-green-100 shadow-sm w-full">
                              ...
                              <span className="text-green-800 text-xs font-bold truncate">
                ...
                <div className="flex gap-3">
                  ...
                  >
                    {claimingTeamId ? (
                      ...
                      <>
                        <span>
                          {teamId ? "Complete Setup" : "Skip & Continue"}
                        </span>
                        <svg
                          className="w-5 h-5"
                          fill="none"
