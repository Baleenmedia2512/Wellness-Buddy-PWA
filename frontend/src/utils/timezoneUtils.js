...
export function istToLocalDate(istTimestamp) {
  ...
  // Remove 'Z' if present and normalize format
  const cleanTimestamp = istTimestamp.replace('Z', '').replace('T', ' ');
 ...
 */
export function formatISTToLocalDateTime(istTimestamp, dateOptions = {}, timeOptions = {}) {
  const localDate = istToLocalDate(istTimestamp);
  if (!localDate) return '';
